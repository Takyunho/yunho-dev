...
- **코드가 UI 부품이 되는 3D 장면**이 있습니다. 첫 화면에서 실제 코드 글자가 기둥처럼 올라가고, 스크롤을 내리면 글자 입자가 브라우저 창, 반응형 기기, 모달, 테마 토글 같은 UI 부품의 표면에 내려앉아 광택 있는 입체로 굳어집니다. 부품은 본문 양옆에 머물다가 마지막에 점으로 풀려 화면 아래에 고이고, 커서가 점에 닿으면 파도처럼 솟았다 가라앉습니다.
...
| `src/components/layout/`   | 헤더, 테마 토글, 부드러운 스크롤, 커스텀 커서, 장면 상태 동기화, 안개 층                                                                |
